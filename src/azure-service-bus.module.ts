import { DynamicModule, Module, Provider, Global, Logger } from '@nestjs/common';
...
export class AzureServiceBusModule {
  private static logger = new Logger('AzureServiceBusModule');
  //registerAsync for read the config file and get the connection string
  static registerAsync(): DynamicModule {
    return {
      ...
      exports: ['AZURE_SERVICE_BUS_CONNECTION'],
    };
  }
  //for root is for the connection string
  static forRoot(options: AzureSBOptions): DynamicModule {
    ...
    if ('connectionString' in options) {
      ...
      clientProvider = {
        provide: 'AZURE_SERVICE_BUS_CONNECTION',
        useValue: new ServiceBusClient(options.fullyQualifiedNamespace, credential),
      };
    }
  ...
  static forRootAsync(options: {
    ...
    const clientProvider: Provider = {
      provide: 'AZURE_SERVICE_BUS_CONNECTION',
      useFactory: async (configService: ConfigService): Promise<ServiceBusClient> => {
        try {
  ...
  static forFeatureAsync(options: {
    ...
    const senderProviders = {
      ...
      useFactory: (client: ServiceBusClient, options: AzureSBSenderReceiverOptions) =>
